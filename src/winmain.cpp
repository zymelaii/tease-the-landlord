...
PCTSTR           szAppName  = TEXT("斗地主");
PCTSTR           szDataFile = TEXT("data");
...
LRESULT CALLBACK
    ...
    switch (message) {
        ...
        case WM_MYBUTTON:
            ...
            return 0;
        case WM_MOUSEMOVE:
            if (!game.IsHumanTurn() || game.GetStatus() != DISCARD) return 0;
...
void ShowImage(HWND hwnd, HDC hdc, int number) {
    ...
    TransparentBlt(
        hdc, -7, -2, 82, 40, hdcmem, 82 * number, 0, 82, 40, RGB(0, 0, 0));
