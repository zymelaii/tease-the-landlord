...
void Scene::ShowScene(HDC hdc) {
    ...
    DrawChars();
    DrawHumanCards(
        hdcScene); // 选择绘制到游戏画面环境中是为了防止二次绘制产生的闪烁
...
void Scene::DrawBackground(void) {
    ...
    // BitBlt(hdcScene, 0, 0, sceneSize.cx, sceneSize.cy, hdcBkg, 0, 0,
    // SRCCOPY);
...
void Scene::DrawComputerCards(void) {
    ...
    if (game->status == GAMEOVER) { // 游戏结束，所有手牌正面显示
        ...
        for (auto rb = game->player[1]->cards.rbegin();
             rb != game->player[1]->cards.rend();
             ++rb) {
            TransparentBlt(
                hdcScene,
                sceneSize.cx - 50 - cardSize.cx / 2,
                65 + 16 * i,
                cardSize.cx,
                cardSize.cy,
                hdcCards,
        ...
        i = 0;
        for (auto rb = game->player[2]->cards.rbegin();
             rb != game->player[2]->cards.rend();
             ++rb) {
            TransparentBlt(
                hdcScene,
                (100 - cardSize.cx) / 2,
        ...
        if (i) { // 先显示牌的背面
            ...
            GetTextExtentPoint32(hdcScene, szText, _tcslen(szText), &size);
            TextOut(
                hdcScene,
                sceneSize.cx - 50 - size.cx / 2,
                65 + (cardSize.cy - size.cy) / 2,
                szText,
                _tcslen(szText));
        }
    ...
    if (game->player[0]->nodiscard) { // 显示“不出”
        ...
        for (auto rb = game->player[0]->discard.cards.rbegin();
             rb != game->player[0]->discard.cards.rend();
             ++rb) { // 显示打出的牌
            TransparentBlt(
                hdcScene,
                (sceneSize.cx - cardSize.cx - 20 * c + 20) / 2 + 20 * i,
                sceneSize.cy - cardSize.cy - 135,
                cardSize.cx,
                cardSize.cy,
                hdcCards,
                cardSize.cx * *rb,
    ...
    if (game->player[1]->nodiscard) { // 显示“不出”
        ...
        for (auto rb = game->player[1]->discard.cards.rbegin();
             rb != game->player[1]->discard.cards.rend();
             ++rb) { // 显示打出的牌
            TransparentBlt(
                hdcScene,
                sceneSize.cx - cardSize.cx - 20 * c - 85 + 20 * i,
                160,
                cardSize.cx,
                cardSize.cy,
                hdcCards,
    ...
    if (game->player[2]->nodiscard) { // 显示“不出”
        ...
        for (auto rb = game->player[2]->discard.cards.rbegin();
             rb != game->player[2]->discard.cards.rend();
             ++rb) { // 显示打出的牌
            TransparentBlt(
                hdcScene,
                105 + 20 * i,
...
void Scene::DrawHumanCards(HDC hdc, int highlight) {
    ...
    int i = 0, y1, c = game->player[0]->cards.size(),
        x = (650 - cardSize.cx - 22 * c + 22) / 2, y = 128 - cardSize.cy - 15;

    for (auto rb = game->player[0]->cards.rbegin();
         rb != game->player[0]->cards.rend();
         ++rb) { // 显示玩家手牌
        if (game->player[0]->selection.cards.find(*rb)
            != game->player[0]
                   ->selection.cards.end()) // 将已选择的牌上移一段，突出显示
            y1 = y - 15;
        else
            y1 = y;
        TransparentBlt(
        ...
        //	Rectangle(hdcScene, x + 22 * i, y1, x + 22 * i+cardSize.cx,
        // y1+cardSize.cy); 	SelectObject(hdcScene, oldpen);
        // DeleteObject(pen);
        //	}
        ++i;
    }
...
void Scene::DrawChars() {
    ...
    switch (game->status) {
        case GETLANDLORD: // 叫地主阶段，显示是否叫地主及分数
            ...
            for (int i = game->questioned, j = game->callbegin; i > 0;
                 --i, j                      = (j + 1) % 3) {
                int x, y;
                ...
                if (game->callscore[j] == 0)
                    TextOut(
                        hdcScene, x, y, szText, wsprintf(szText, TEXT("不叫")));
                else
                    TextOut(
                        hdcScene,
                        x,
                        y,
                        szText,
                        wsprintf(szText, TEXT("%d分"), game->callscore[j]));
        ...
        case NOTSTART:           // 显示电脑和玩家字样
            ...
            TextOut(
                ...
                10,
                szText,
                wsprintf(szText, TEXT("%s"), TEXT("电脑")));
            TextOut(
                hdcScene,
                50,
                10,
        ...
        case GAMEOVER:
            ...
            TextOut(
                ...
                wsprintf(
                    szText,
                    TEXT("%s"),
                    game->landlord == game->player[0] ? TEXT("地主")
                                                      : TEXT("农民")));
            TextOut(
                hdcScene,
                sceneSize.cx - 50,
                ...
                wsprintf(
                    szText,
                    TEXT("%s"),
                    game->landlord == game->player[1] ? TEXT("地主")
                                                      : TEXT("农民")));
            TextOut(
                hdcScene,
                50,
                ...
                wsprintf(
                    szText,
                    TEXT("%s"),
                    game->landlord == game->player[2] ? TEXT("地主")
                                                      : TEXT("农民")));
            SelectObject(hdcScene, oldfont);
            DeleteObject(hfont);
            ...
            TextOut(
                ...
                szText,
                wsprintf(
                    szText, TEXT("%d分 %d倍"), game->basescore, game->times));
    ...
    hfont = CreateFont(
        20, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, TEXT("宋体"));
    oldfont   = (HFONT)SelectObject(hdcScene, hfont);
    textalign = SetTextAlign(hdcScene, TA_CENTER);
    // 各玩家分数
    TextOut(
...
int Scene::PointInWhich(POINT point) {
    ...
    for (int i = c; i > 0 && b != human->cards.end(); --i, ++b) {
        ...
        if (PtInRect(&rect, point)) return *b;
    }
    return -1;
}
...
void Scene::MouseMove(POINT point) {
    // BitBlt(hdcScene, 0, 0, sceneSize.cx, sceneSize.cy, hdcBkg, 0, 0,
    // SRCCOPY); DrawComputerCards(); DrawHumanCards(PointInWhich(point));
    // DrawChars();
