...
void Game::InitGame() {
    ...
    times                  = 1;
    for (int i = 0; i < 3; ++i) { player[i]->NewGame(); }
    cardheap.RandCards();
    status = GETLANDLORD;
}
...
void Game::LoadPlayerScore() {
    ...
    if (!in) return;
    for (int i = 0; i < 3; ++i) in >> player[i]->score;
}
...
void Game::StorePlayerScore() {
    ...
    if (!out) return;
    for (int i = 0; i < 3; ++i) out << player[i]->score << "\n";
}
...
void Game::SendCard(void) {
    ...
    for (int i = 0; i < 3; ++i) landlordcard[i] = cardheap.GetCard();
}
...
void Game::SendScore(int result) {
    ...
    ++questioned;
    scene->ShowScene(hMainWnd);
    if (landlord) status = SENDLANDLORDCARD;
    SetTimer(hMainWnd, 1, 500, NULL);
}

// 发地主牌
void Game::SendLandlordCard() {
    for (auto mem : landlordcard) landlord->AddCard(mem);
...
void Game::Discard() {
    ...
    if (curplayer == player[0]) { // 当前玩家为人
        ...
        if (curplayer->Discard()) lastone = curplayer;
...
void Game::GameOver() {
    ...
    if (landlord->cards.size()) { // 农民胜利
        ...
        if (player[0] != landlord) IsPeopleWin = true;
    } else { // 地主胜利
        landlord->score     += score * 2;
        ProPlayer()->score  -= score;
        NextPlayer()->score -= score;
